...
@interface LinkMicViewController ()<UITextFieldDelegate, TXLivePushListener, StreamUrlScannerDelegate, ScanQRDelegate, BeautySettingPanelDelegate>
...
-(void) initPlayItem: (int)index
{
    ...
    if (index == 0 || index == 1)
    {
        videoView.center = CGPointMake(self.view.bounds.size.width * (5 - index * 2) / 6, CGRectGetMinY(_btnBeauty.frame) - 10 - height / 2);
    }
    else
    {
        videoView.center = CGPointMake(self.view.bounds.size.width * 5 / 6, CGRectGetMinY(_btnBeauty.frame) - 10 - height / 2- height - 10);
    }

    LinkMicPlayItem * playItem = [[LinkMicPlayItem alloc] init];
//    if (index == 0)
//    {
//        playItem.videoView = _videoPreview;
//        _smallVideoView = videoView;
//    }
//    else
    {
...
-(void) clickPushType:(UIButton*) btn
{
    ...
    if ([_txLivePublisher isPublishing]) {
        if (_isMainPublisher) {
            [_txLivePublisher setVideoQuality:VIDEO_QUALITY_LINKMIC_MAIN_PUBLISHER adjustBitrate:NO adjustResolution:NO];
        }
        else {
            [_txLivePublisher setVideoQuality:VIDEO_QUALITY_LINKMIC_SUB_PUBLISHER adjustBitrate:NO adjustResolution:NO];
        }
    }
}
...
-(BOOL)startRtmp{
    ...
    if(_txLivePublisher != nil)
    {
        ...
        pushConfig.connectRetryCount = 5;
        pushConfig.connectRetryInterval = 1;
        ...
        if (!_isPreviewing) {
            ...
            //[_txLivePublisher startPreview:_smallVideoView];
            _isPreviewing = YES;
        }
